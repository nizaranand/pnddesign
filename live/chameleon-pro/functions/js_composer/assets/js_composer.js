    ...
    $.wpb_composer = {
        ...
        galleryImagesControls: function() {
            $('.gallery_widget_add_images').live("click", function(e) {
                ...
                tb_show('Add/remove picture', 'media-upload.php?type=image&post_id=' +  post_id +'&tab=composer_images&single_image=' + ($(this).attr('use-single')=='true' ? 'true' : 'false') + '&selected_ids=' + encodeURIComponent(selected_ids) + '&TB_iframe=true&height=343&width=800');
...
function wpbTabsInitCallBack(element) {
	element.find('.wpb_tabs_holder').not('.wpb_initialized').each(function(index) {
		...
		delete_btn.click( function(e) {
			...
			var answer = confirm ('Press OK to delete "'+tab_name+'" tab, Cancel to leave');
			if ( answer ) {
...
function wpbTabsGenerateShortcodeCallBack(current_top_level, inner_element_count) {
	...
	output = '[vc_tab title="'+tab_title+'" tab_id="' + (+new Date() + '-' + Math.floor(Math.random()*11)) +'"] %inner_shortcodes [/vc_tab]';
	return output;
}
